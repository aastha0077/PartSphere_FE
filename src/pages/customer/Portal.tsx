...
import { Package, Calendar, History, Search, ShoppingBag, Plus, Clock, Sparkles, AlertCircle, Car, CreditCard, CheckCircle, CheckCircle2 } from 'lucide-react';
    ...
    <div className="animate-fade-in" style={{ maxWidth: '1400px', margin: '0 auto', padding: '2rem 1.5rem' }}>
      ...
      <div className="glass-card mb-8" style={{
        ...
        <div style={{ display: 'flex', gap: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
          <button onClick={() => navigate('/customer/booking', { state: { openBooking: true } })} style={{ padding: '12px 24px', background: 'var(--accent-gradient)', color: 'white', borderRadius: '12px', display: 'flex', alignItems: 'center', gap: '8px', fontWeight: '700', boxShadow: '0 8px 20px -8px rgba(99, 102, 241, 0.6)' }}>
                      ...
                      return rows.length === 0 ? (
                        ...
                        rows.map((order: any) => (
                          <tr key={order.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.02)' }} className="table-row-hover">
                            <td style={{ padding: '6px 12px', fontFamily: 'monospace', fontWeight: '600', fontSize: '0.875rem', color: 'white' }}>#{order.id}</td>
                  ...
                  <div key={app.id} className="glass-card" style={{ border: '1px solid var(--glass-border)', padding: '1.5rem' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem' }}>
                      ...
                      <span style={{
                        ...
                        color: app.status === 'Completed' ? '#10b981' : app.status === 'Cancelled' ? '#ef4444' : app.status === 'Booked' || app.status === 'Confirmed' ? '#6366f1' : '#f59e0b'
                      }}>
                        {app.status}
                      </span>
                    </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--text-secondary)' }}>
                        <Clock size={16} />
                        <span style={{ fontWeight: '600' }}>{(() => { const d = parseUTCDate(app.date); return `${d.toLocaleDateString()} at ${d.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}`; })()}</span>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'var(--text-secondary)' }}>
                        <ShoppingBag size={16} />
