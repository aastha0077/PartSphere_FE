...
const CustomerBooking = () => {
  ...
  const sortedHistory = [...appointments].sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
                  ...
                  {dayAppointments.length === 0 ? (
                    ...
                    <div key={apt.id} style={{ padding: '1rem', background: 'rgba(255,255,255,0.02)', borderRadius: '12px', borderLeft: `3px solid ${apt.status === 'Completed' ? '#10b981' : '#6366f1'}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                        <p style={{ fontWeight: '800', fontSize: '1rem', color: 'white', width: '60px' }}>{new Date(apt.date).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                ...
                {sortedHistory.length === 0 ? (
                  ...
                  <div key={apt.id} className="table-row-hover" style={{ padding: '1.25rem', background: 'rgba(255,255,255,0.01)', borderRadius: '16px', border: '1px solid rgba(255,255,255,0.03)', display: 'grid', gridTemplateColumns: '100px 1fr 1fr 120px', alignItems: 'center', gap: '1.5rem' }}>
                    <div style={{ textAlign: 'center' }}>
                       ...
                       <p style={{ fontWeight: '700', fontSize: '0.85rem' }}>{new Date(apt.date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}</p>
                       <p style={{ fontSize: '0.7rem', color: 'var(--text-muted)' }}>{new Date(apt.date).getFullYear()}</p>
                    </div>
                    <div>
          ...
          <AnimatePresence mode="wait">
            ...
            {step === 3 && (
              <motion.div key="s3" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}>
                <h3 style={{ fontSize: '1.1rem', fontWeight: '800', marginBottom: '1.25rem' }}>Confirm Booking</h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  <div style={{ padding: '1rem', background: 'rgba(99, 102, 241, 0.05)', borderRadius: '12px', border: '1px solid rgba(99, 102, 241, 0.1)', fontSize: '0.85rem' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                      <div><p style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginBottom: '2px' }}>VEHICLE</p><p style={{ fontWeight: '700' }}>{vehicles.find(v => v.id.toString() === formData.vehicleId)?.brand} {vehicles.find(v => v.id.toString() === formData.vehicleId)?.model}</p></div>
                      <div><p style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginBottom: '2px' }}>SERVICE</p><p style={{ fontWeight: '700' }}>{formData.serviceType}</p></div>
                      <div style={{ gridColumn: 'span 2' }}><p style={{ fontSize: '0.65rem', color: 'var(--text-muted)', marginBottom: '2px' }}>SCHEDULED FOR</p><p style={{ fontWeight: '700' }}>{selectedDate.toLocaleDateString(undefined, { weekday: 'short', month: 'short', day: 'numeric' })} at {formData.timeSlot}</p></div>
                    </div>
                  </div>
